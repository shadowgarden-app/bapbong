...
async function sampleBytes(): Promise<Uint8Array> {
  const doc = schema.node('doc', null, [
    schema.node('paragraph', null, [
      schema.text('Bao gia dich vu thanh lap cong ty.'),
    ]),
    schema.node('paragraph', null, [
    ...
    schema.node('paragraph', null, [
      schema.text('Lien he: 0336697918. Tong phi: chua VAT.'),
    ]),
    schema.node('paragraph', null, [
...
async function openSession(
  ...
  return HeadlessSession.open(await sampleBytes(), {
    name: 'sample.docx',
    onSave,
  });
}
...
describe('HeadlessSession', () => {
  ...
  it('rejects an ambiguous anchor, then honors occurrence', async () => {
    const s = await openSession();
    await expect(s.replaceText('Tong phi', 'Thanh tien')).rejects.toThrow(
      AnchorError,
    );
    await expect(s.replaceText('Tong phi', 'Thanh tien')).rejects.toThrow(
      /matches 2 times/,
  ...
  it('rejects a missing anchor with a teaching message', async () => {
    const s = await openSession();
    await expect(s.replaceText('khong ton tai', 'x')).rejects.toThrow(
      /not found/,
    );
  });
  ...
  it('enforces optimistic locking via expectedVersion', async () => {
    ...
    await expect(
      s.replaceText('VND', 'USD', { expectedVersion: 'v1' }),
  ...
  it('inserts paragraphs before/after an anchor and at document_end', async () => {
    const s = await openSession();
    await s.insertContent('Ghi chu dau trang', {
      position: 'before',
      text: 'Bao gia dich vu',
    });
  ...
  it('updateImage resizes/rotates in one step, teaching on bad anchors', async () => {
    ...
    const res = await s.updateImage(
      3,
      0,
      { width: 200, rotation: 450.5 },
      { expectedVersion: v1 },
    );
    expect(res.docVersion).toBe('v2');
    ...
    await expect(s.updateImage(0, 0, { width: 10 })).rejects.toThrow(
      /has no images/,
    );
    await expect(s.updateImage(3, 5, { width: 10 })).rejects.toThrow(/0-0/);
    await expect(s.updateImage(99, 0, { width: 10 })).rejects.toThrow(
      AnchorError,
    );
    await expect(
      s.updateImage(3, 0, { width: 10 }, { expectedVersion: 'v1' }),
    ).rejects.toThrow(VersionConflictError);
  });
...
describe('wire (RemoteSession ↔ executeOp)', () => {
  ...
  async function remotePair() {
    ...
    const remote = new RemoteSession(
      async (op: SessionOpName, args: unknown[]) => {
  ...
  it('round-trips reads and mutations across the hop', async () => {
    ...
    expect(snap.blocks).toHaveLength(4);
    const res = await remote.replaceText('1.500.000', '8.888.888', {
      expectedVersion: snap.docVersion,
    });
    expect(res.docVersion).toBe('v2');
    expect((await remote.snapshot()).blocks[1].text).toBe(
      'Tong phi: 8.888.888 VND.',
    );
  ...
  it('revives contract errors by name across the hop', async () => {
    const { remote } = await remotePair();
    await expect(remote.replaceText('Tong phi', 'x')).rejects.toThrow(
      AnchorError,
    );
    await expect(
      remote.replaceText('VND', 'x', { expectedVersion: 'v9' }),
    ).rejects.toThrow(VersionConflictError);
  });
...
describe('createMcpServer (end-to-end over MCP)', () => {
  async function connect() {
    ...
    const provider = {
      get: async (id?: string) =>
        id === undefined || id === 'sample' ? session : null,
    };
  ...
  it('reads the document and edits it through tool calls', async () => {
    ...
    const doc = JSON.parse(
      text(await client.callTool({ name: 'get_document', arguments: {} })),
    );
    ...
    const res = await client.callTool({
      ...
      arguments: {
        old_text: '1.500.000',
        new_text: '3.333.333',
        expectedVersion: doc.docVersion,
      },
    });
  ...
  it('updates an image through the tool (and rejects an empty change)', async () => {
    ...
    expect(after.blocks[3].images[0]).toMatchObject({
      width: 150,
      height: 75,
      rotation: 45,
    });
  ...
  it('resolves documentId through the provider', async () => {
    ...
    const ok = await client.callTool({
      name: 'get_document',
      arguments: { documentId: 'sample' },
    });
    expect(ok.isError).toBeFalsy();
    const missing = await client.callTool({
      name: 'get_document',
      arguments: { documentId: 'other' },
    });
  ...
  it('registers tools for ONE document — discovering others is up to the host', async () => {
    ...
    expect((await extended.listTools()).tools.map((t) => t.name)).toContain(
      'list_documents',
    );
  });
  ...
  it('singleDocumentProvider answers only for the omitted id', async () => {
    ...
    expect(
      (await client.callTool({ name: 'get_document', arguments: {} })).isError,
    ).toBeFalsy();
    const other = await client.callTool({
      name: 'get_document',
      arguments: { documentId: 'x.docx' },
    });
...
describe('doc-scoped versions', () => {
  it('prefixes docVersion with the document id so locks cannot cross documents', async () => {
    ...
    await expect(
      a.replaceText('1.500.000', '9', { expectedVersion: 'a.docx:v1' }),
    ).resolves.toBeTruthy();
  });
