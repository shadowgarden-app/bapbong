...
describe('resizeRect from a corner (an embedded object`s only handle)', () => {
  it('keeps the aspect ratio, so an equation preview never distorts', () => {
    const r = resizeRect(base, 'se', 60, 0, false);
    expect(r.width / r.height).toBeCloseTo(base.width / base.height, 5);
  });
});
